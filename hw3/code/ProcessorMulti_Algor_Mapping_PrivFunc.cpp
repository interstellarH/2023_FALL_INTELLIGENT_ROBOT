...
bool DECOFUNC(processMultiInputData)(void * paramsPtr, void * varsPtr, QVector<QVector<void *> > inputParams, QVector<QVector<void *> > inputData, void * outputData, QList<int> & outputPortIndex)
{
    ...
    */
    ...
    //==============================================================================
        ...
        double angle = i * inputparams_0.front()->res * 3.14159261314 / 180.0;//计算得到当前处理激光束在激光雷达坐标系中角度
        lx = dis * cos(angle);
        ly = dis * sin(angle);
        ...
        double laser_aL_rad = inputparams_0.front()->aL * 3.14159261314 / 180.0;//转换为弧度
        ...
        //         else misses[(*c).y][(*c).x]++;
        //         // limit to [-300,300]
        //         vars->map[(*c).y][(*c).x] = -300 + 600 * (hits[(*c).y][(*c).x]/(hits[(*c).y][(*c).x]+misses[(*c).y][(*c).x]));
        //     }
        // }
    //==============================================================================
    //-----------------Please edit above--------------------------------------------
