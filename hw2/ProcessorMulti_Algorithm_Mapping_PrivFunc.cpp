...
bool DECOFUNC(processMultiInputData)(void * paramsPtr, void * varsPtr, QVector<QVector<void *> > inputParams, QVector<QVector<void *> > inputData, void * outputData, QList<int> & outputPortIndex)
{
    ...
    */
    ...
    const double pi = 3.1415926535897934384626;
    double posx = inputdata_0.front()->x * 10.0;
    double posy = inputdata_0.front()->y * 10.0;
    double pos_ori = inputdata_0.front()->orientation;

    for (int i = 0; i < inputdata_1.front()->datasize ; i ++) {

        // 计算得到单个激光点的距离(m)和角度(rad)
        double dis = inputdata_1.front()->data[i] / 10.0;
        if (dis==0)
            continue; // get rid of invalid points

        double angle = pi * i / 360.0;
        double lx = dis * cos(angle);
        double ly = dis * sin(angle);

        // 激光点在全局坐标系中的位置 单位m
        double gx, gy;
        LP2GP(lx, ly, posx, posy, pos_ori, &gx, &gy);
