...
/**************************************************填写以下部分***************************************************************/
    ...
    // --------------------处理编码器数据溢出,并将单位转换为m-----------------------------------------------
    //consider the reverse case, can't write this way
    //if(deltaPluse < 0) //should be increasing
    //    deltaPluse += pluseMax;
    deltaPluse=(deltaPluse + pluseMax) % pluseMax;
    double distance = disPerPluse * deltaPluse;
    // --------------------处理编码器数据溢出,并将单位转换为m-----------------------------------------------


    // --------------------计算小车当前时刻的位置，并更新last数据--------------------
    double averageOri  = vars->lastori + (currentOri-vars->lastori) / 2.0;
    ...
    // ----------------------填充outputdata---------------------------------------------
    outputdata->orientation = currentOri;
